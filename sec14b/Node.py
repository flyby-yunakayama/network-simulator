...
from sec14b.Packet import Packet, UDPPacket, TCPPacket, ARPPacket, DNSPacket, DHCPPacket, HTTPPacket
...
class Node:
    ...
    def is_server(self):
        return getattr(self, 'is_server_node', False)
            ...
            if packet.header["destination_ip"] == self.ip_address:
                ...
                if isinstance(packet, HTTPPacket):
                    if packet.payload:
                        if self.is_server():
                            # サーバ側でHTTPリクエストを処理
                            self.handle_http_request(packet)
                        else:
                            # クライアント側でHTTPレスポンスを処理
                            self.handle_http_response(packet)

            else:
                self.network_event_scheduler.log_packet_info(packet, "dropped", self.node_id)

    def handle_http_request(self, packet):
        """
        HTTPリクエストを処理し、レスポンスを送信します。
        """
        http_request_data = packet.payload.decode()
        request_lines = http_request_data.split('\r\n')
        request_line = request_lines[0]
        method, path, _ = request_line.split(' ')

        if self.network_event_scheduler.tcp_verbose:
            print(f"Received HTTP {method} request for {path} from {packet.header['source_ip']}")

        # リクエストされたリソースを取得（ここではダミーデータを使用）
        resource_data = b"Hello, World! This is the content of the requested resource."

        # HTTPレスポンスメッセージの作成
        status_line = "HTTP/1.1 200 OK\r\n"
        headers = "Content-Type: text/plain\r\n" + f"Content-Length: {len(resource_data)}\r\n"
        http_response_data = (status_line + headers + "\r\n").encode() + resource_data

        # HTTPレスポンスパケットの作成
        response_packet = HTTPPacket(
            source_mac=self.mac_address,
            destination_mac=packet.mac_header["source_mac"],
            source_ip=self.ip_address,
            destination_ip=packet.ip_header["source_ip"],
            ttl=64,
            dscp=packet.ip_header["dscp"],
            fragment_flags={},
            fragment_offset=0,
            header_size=40,
            payload_size=len(http_response_data),
            network_event_scheduler=self.network_event_scheduler,
            source_port=packet.tcp_header["destination_port"],
            destination_port=packet.tcp_header["source_port"],
            sequence_number=self.tcp_connections[(packet.ip_header["source_ip"], packet.tcp_header["source_port"])]["sequence_number"],
            acknowledgment_number=packet.tcp_header["sequence_number"] + len(packet.payload),
            flags="ACK",
            http_data=http_response_data
        )
        ...
        # シーケンス番号を更新
        self.tcp_connections[(packet.ip_header["source_ip"], packet.tcp_header["source_port"])]["sequence_number"] += len(http_response_data)

    def handle_http_response(self, packet):
        """
        HTTPレスポンスを処理します（クライアント側）。
        """
        http_response_data = packet.payload.decode()
        response_lines = http_response_data.split('\r\n')
        status_line = response_lines[0]
        _, status_code, reason = status_line.split(' ', 2)

        if self.network_event_scheduler.tcp_verbose:
            print(f"Received HTTP response: {status_code} {reason}")
    ...
    def initialize_connection_info(self, connection_key=None, state='CLOSED', sequence_number=0, acknowledgment_number=0, data=b''):
        ...
        self.tcp_connections[connection_key] = {
            ...
            'sequence_number_base': sequence_number,  # 送信開始時のシーケンス番号を保存
    ...
    def start_http_request(self, destination_url, resource_path, start_time=0, headers=None):
        """
        HTTP GETリクエストを開始します。

        :param destination_url: 宛先のURLまたはIPアドレス
        :param resource_path: 取得したいリソースのパス（例：'/index.html'）
        :param start_time: リクエスト開始時間
        :param headers: HTTPヘッダーの辞書
        """
        def attempt_to_start_http():
            if self.is_valid_cidr_notation(destination_url):
                self.send_http_request(destination_url, resource_path, headers)
            else:
                destination_ip = self.resolve_destination_ip(destination_url)
                if destination_ip is None:
                    self.send_dns_query_and_set_http(destination_url, resource_path, headers)
                else:
                    self.send_http_request(destination_ip, resource_path, headers)

        self.network_event_scheduler.schedule_event(start_time, attempt_to_start_http)

    def send_http_request(self, destination_ip, resource_path, headers=None):
        """
        HTTP GETリクエストを送信します。

        :param destination_ip: 宛先のIPアドレス
        :param resource_path: リクエストするリソースのパス
        :param headers: HTTPヘッダーの辞書
        """
        source_port = self.select_random_port()
        destination_port = 80  # HTTPのデフォルトポート

        # HTTPリクエストメッセージの作成
        http_request_line = f"GET {resource_path} HTTP/1.1\r\n"
        http_headers = headers or {}
        http_headers_str = ''.join(f"{k}: {v}\r\n" for k, v in http_headers.items())
        http_data = (http_request_line + http_headers_str + "\r\n").encode()
    ...
    def send_dns_query_and_set_http(self, destination_url, resource_path, headers=None):
        """
        DNSクエリを送信し、HTTPリクエストをスケジュールします。
        """
        if destination_url not in self.waiting_for_dns_reply:
            self.waiting_for_dns_reply[destination_url] = []
        self.waiting_for_dns_reply[destination_url].append(('HTTP', resource_path, headers))
    ...
    def on_dns_response_received(self, query_domain, resolved_ip):
        ...
        if query_domain in self.waiting_for_dns_reply:
            for parameters in self.waiting_for_dns_reply[query_domain]:
                if parameters[0] == 'HTTP':
                    _, resource_path, headers = parameters
                    self.send_http_request(resolved_ip, resource_path, headers)
                else:
