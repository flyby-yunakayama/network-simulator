...
class HTTPPacket(TCPPacket):
    def __init__(self, http_data, **kwargs):
        super().__init__(**kwargs)
        self.http_data = http_data  # HTTPのデータ（リクエストまたはレスポンス）
        self.size += len(http_data)  # パケットサイズにHTTPデータのサイズを加算
    ...
    def __str__(self):
        source_ip = self.ip_header.get("source_ip", "不明")
        destination_ip = self.ip_header.get("destination_ip", "不明")
        return f'HTTPPacket(送信元IP: {source_ip}, 宛先IP: {destination_ip}, シーケンス番号: {self.tcp_header["sequence_number"]}, ACK番号: {self.tcp_header["acknowledgment_number"]}, フラグ: {self.tcp_header["flags"]}, HTTPデータサイズ: {len(self.http_data)})'
