...
class Node:
    ...
    def start_ftp_transfer(self, destination_url, file_path, start_time=0, protocol="TCP", dscp=0):
        """
        FTP転送を開始します。

        :param destination_url: 宛先のURLまたはIPアドレス
        :param file_path: 転送するファイルのパス
        :param start_time: 転送開始時間（デフォルトは0）
        :param protocol: 使用するプロトコル（デフォルトは"TCP"）
        :param dscp: DSCP値（デフォルトは0）
        """
        def attempt_to_start_ftp():
            if self.is_valid_cidr_notation(destination_url):
                destination_ip = destination_url
            else:
                destination_ip = self.resolve_destination_ip(destination_url)
                if destination_ip is None:
                    self.send_dns_query_and_set_ftp(destination_url, file_path, start_time, protocol, dscp)
                    return
            self.simulate_ftp_transfer(destination_ip, file_path, start_time, protocol, dscp)

        self.network_event_scheduler.schedule_event(start_time, attempt_to_start_ftp)

    def simulate_ftp_transfer(self, destination_ip, file_path, start_time, protocol, dscp):
        """
        実際のファイル転送をシミュレートし、進行状況を追跡します。

        :param destination_ip: 宛先のIPアドレス
        :param file_path: 転送するファイルのパス
        :param start_time: 転送開始時間
        ...
        if connection_key not in self.tcp_connections:
            self.initialize_connection_info(connection_key=connection_key, sequence_number=randint(1, 10000), data=data)
            self.tcp_connections[connection_key]['transfer_info'] = transfer_info

        # TCP接続の情報を設定
        self.tcp_connections[connection_key]['traffic_info'] = {
            'payload_size': 1000,  # ペイロードサイズを設定
            'header_size': 40,
            'bitrate': None,  # ビットレート制限をしない
            'burstiness': 1.0,
            'next_sequence_number': self.tcp_connections[connection_key]['sequence_number']
        }
